...
class GetSubgraph(nn.Module):
    ...
    def forward(self, m_node,d_node, node_adj, rel_adj):
        ...
        node_get_pre.index_put_(indices=[node_edge_index[0],node_edge_index[1]],values=torch.tensor(1.).cuda())
        node_get_pre.index_put_(indices=[md_node,dm_node],values=torch.tensor(0.).cuda())
